...
% wikipedia link parity bit (footnote)
...
%Dieser Angabe liegt ein Script namens \verb|run_tests| bei, welches Sie zum Testen Ihrer Implementierung verwenden können.
%Das Script führt Ihren Server und Client mit verschiedenen Testfällen aus.
%Beim Aufruf übergeben Sie dem Script den Pfad zum Server- und zum Client-Programm.
...
%Sie können das Script auch bearbeiten und eigene Testfälle hinzufügen.

%Außerdem liegt der vom Institut implementierte Server als Referenz bei.
%Bei der Ausführung des \verb|run_tests|-Script können Sie diesen anstelle Ihres eigenen Server übergeben
%und so vergleichen ob der von Ihnen implementierte Server sich gleich verhält.
%Außerdem können Sie Ihren Client damit testen.
