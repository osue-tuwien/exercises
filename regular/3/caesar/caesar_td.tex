...
Das Programm \osueprog{readin} liest zeilenweise aus einem als Argument
...
auf \osueglvar{stdout} ausgegeben, und die n"achste Zeile wird gelesen.

\osueprog{caesar} soll die n"achste Zeile erst dann lesen, wenn
\osueprog{readin} sie vollst"andig ins Shared Memory geschrieben hat und
\osueprog{readin} soll erst dann die n"achste Zeile ins Shared Memory schreiben,
wenn die vorherige schon von \osueprog{caesar} gelesen wurde.

Sobald \osueprog{readin} die letzte Zeile des Eingabefiles verarbeitet hat,
schreibt es einen frei w"ahlbaren String in das Shared Memory, der
\osueprog{caesar} anzeigt, dass alle Arbeit getan ist. Darauf sollen beide
...
Erstellen Sie z.B. ein Testfile \osuefilename{t1} mit folgendem Inhalt: 
\begin{verbatim}
...
\begin{verbatim}
  ...
  caesar 15 > t2
\end{verbatim}
Dann haben Sie in \osuefilename{t2} die verschl"usselte Version von \osuefilename{t1}.
