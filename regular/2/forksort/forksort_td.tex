...
Das Programm soll die Daten von \osueglvar{stdin} lesen. Die erste Zeile muss eine
...
\item Ist die Anzahl größer Eins, dann erstellen Sie vier Unnamed-Pipes (zwei
  je Kind) und forken diese zwei Kindprozesse. Verwenden Sie \osuefunc{dup2(2)} und
  \osuefunc{execlp(3)} um \osueglvar{stdin}/\osueglvar{stdout} der neuen Prozesse auf eigene Streams
...
\item Verwenden Sie \osuefunc{wait(2)} oder \osuefunc{waitpid(2)}, um den Exit-Status
...
\osuefunc{strcmp(3)} als Merge-Implementation sollte jetzt noch reichen. Wenn das
...
\osueglvar{stderr}, da die \osueglvar{stdout} in den meisten Fällen verbogen ist.
